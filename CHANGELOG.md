...
### Added
- Демо-режим на вкладке «Рекомендация» — реальный сохранённый ответ API
  (`src/lib/demoData.js`) показывается, когда backend недоступен (GitHub Pages без
  локального сервера), вместо тупика с ошибкой подключения.
- Вкладка «Пакетный расчёт» (JSON-редактор поверх уже существующего
  `/skills/recommend_price/batch`) и вкладка «ROI» (калькулятор окупаемости, чистый
  клиентский расчёт).
- Code splitting дашборда: `recharts` вынесен из основного бандла в отдельный чанк,
  грузится по требованию. Основной бандл: 622 KB → 217 KB.
...
### Известные открытые пункты (не сделано, не забыто)
- Production pilot package под конкретную конфигурацию 1С (УТ11/ERP/КА/Розница) —
  нужен реальный заказчик, чтобы знать, под какую конфигурацию адаптировать.
- Fact-control отчётность (прогноз vs факт) — есть `scripts/backtest_elasticity.py`
  как инструмент, но нечего скармливать: нет реальной истории цена/спрос.
- Сбор реальных данных для обучения модели эластичности — то же самое ограничение.
