...
- **Пакетный расчёт** — JSON-редактор до 200 позиций сразу, вызывает
  `/skills/recommend_price/batch`; некорректная позиция не роняет весь пакет.
- **ROI** — калькулятор экономического эффекта (срок окупаемости, доп. прибыль от uplift)
  для демонстрации заказчику. Чистый клиентский расчёт, backend не нужен.
- **Демо-режим** на вкладке «Рекомендация» — если backend недоступен (например, первый
  визит на GitHub Pages без локального сервера), можно посмотреть на реальном
  сохранённом примере ответа (`src/lib/demoData.js`, снят живым вызовом API, не выдуман)
  вместо пустого экрана с ошибкой подключения.
