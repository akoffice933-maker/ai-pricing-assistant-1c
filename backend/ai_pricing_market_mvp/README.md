...
**Прямо: цифры, которые сейчас выдаёт сервис, не валидированы.** `market_demand_index` без внешнего
источника данных вырождается в `seasonality_index` (см. [`docs/MARKET_DATA_METHOD.md`](../../docs/MARKET_DATA_METHOD.md)),
а базовая эластичность — калиброванная догадка, ни разу не проверенная на реальной паре цена/спрос.
Подробный честный разбор — в [корневом README, раздел «Ограничения модели»](../../README.md#ограничения-модели--честно).
...
Пока эластичность — калиброванная эвристика (не обученная модель), нужен способ
проверять её на истории до пилота. `scripts/backtest_elasticity.py` берёт CSV с
историческими парами «цена → фактический спрос» и сравнивает их с прогнозом
`DemandCurveSkill` (MAE, MAPE, систематическое смещение, разбивка по категориям):

```bash
python scripts/backtest_elasticity.py backend/ai_pricing_market_mvp/examples/backtest_sample.csv
```

`examples/backtest_sample.csv` — **синтетический** пример на 8 строк, нужен только
